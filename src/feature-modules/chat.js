	...
	tabContents: `
		...
		<div class="rpht-option-block">
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="snapRoomListEnable" dataName="snapRoomList"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section">
				...
				<select style="float: right; width: 110px;" id="chatColorSelection" dataName="colorStylizing">
			...
			<div class="rpht-option-section">
				...
				<select style="float: right; width: 110px;" id="unreadMarkerSelection" dataName="unreadMarkerSelection">
			...
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="chatmsgPaddingEnable" dataName="msgPadding"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="hideCommandWindowEnable" dataName="hideCommandWindow"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section option-section-bottom">
				...
				<label class="switch"><input type="checkbox" id="enableTabSwitch" dataName="enableTabSwitch"><span class="rpht-slider round"></span></label>
		...
		<div class="rpht-option-block">
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="notifyPingEnable" dataName="enablePings"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="notifyNotificationEnable" dataName="pingNotify"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="pingExactMatch" dataName="exact"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="pingCaseSense" dataName="case"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="pingItalicsEnable" dataName="italics"><span class="rpht-slider round"></span></label>
			</div>
			<div class="rpht-option-section option-section-bottom">
		...
		<div class="rpht-option-block">
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="trackSession" dataName="trackSession"><span class="rpht-slider round"></span></label>
			...
			<div class="rpht-option-section">
				...
				<label class="switch"><input type="checkbox" id="joinFavEnable" dataName="joinFavorites"><span class="rpht-slider round"></span></label>
...
const DICE_ROLL_POPUP_HTML = `<div id="diceRollerPopup" class="rpht-tooltip-common">
		...
		<br><br>
		<button id="dieRollButton">Let's roll!</button>
		<hr style="margin-top: 6px; margin-bottom: 6px;">
		<button id="coinFlipButton">Flip a coin!</button>
	</div>`;
...
function init() {
	...
	/* General Options */
	$('#snapRoomListEnable').change((ev) => {
		saveSetting(ev);
		rph.roomsJoined.forEach(setupSnapRoomList)
	});
	$('#chatColorSelection').change(saveSetting);
	$('#unreadMarkerSelection').change(saveSetting);
	$('#chatmsgPaddingEnable').change(saveSetting);
	$('#hideCommandWindowEnable').change(saveSetting);
	$(`#enableTabSwitch`).change((ev) => {
		saveSetting(ev);
	...
	$('#joinFavEnable').change(saveSetting);
	$('#favAdd').click(() => {
	...
	socket.on('confirm-room-join', roomSetup);
	...
	/* Setup opening dialog dismiss timer */
...
function processRNG(msgLine) {
	...
	else if (msgLine.search('flips' > -1)) {
		const outcomes = ['heads', 'tails'];
		let outcome = outcomes[LcgRng(SEED) % 2];
		validResult = (msgLine.search(outcome) >- 1);
	}
	...
	if (validResult === false) {
		...
		msgLine += ` <span style="background:#4A4; color: #FFF;" title="This result is good">&#9745;</span>`
	}
	return msgLine;
}
...
function processPings(msgData, message) {
	...
	if (isMod) {
		...
		if (alertRegex) {
			message = message.replace(alertRegex, `<span style="${ALERT_HIGHLIGHT}">${message.match(alertRegex)}</span>`)
			moddingModule.playAlert();
			notificationTrigger = 2;
		}
	}
	else if (chatSettings.enablePings) {
		let testRegex = matchPing(message)
		if (testRegex) {
			message = message.replace(testRegex, `<span style="${pingHighlightText}">${message.match(testRegex)}</span>`)
...
function updateChatLog(roomName, message) {
	...
	if (chatRoomLogs[roomName].length == MAX_CHAT_LOGS) {
		chatRoomLogs[roomName].shift();
	}
	chatRoomLogs[roomName].push(message);
}
...
function intputChatText(ev, User, Room) {
	...
	else if (message.length === 0) { return; } 
	else if (ev.keyCode !== 13 || ev.shiftKey === true || ev.ctrlKey === true) { return; }
	else if (ev.keyCode === 13 && (ev.shiftKey === true || ev.ctrlKey === true)) { inputTextarea.val(inputTextarea.val() + "\n"); }
...
function resizeChatTabs() {
	...
	setTimeout(() => {$(window).resize(resizeChatTabs);}, DEBOUNCE_TIMEOUT_MS)
}

function scrollToRoom(ev) {
	let roomNameCss = $(ev.target)[0].classList[1];
	roomNameCss = roomNameCss.substr(4, roomNameCss.length);

	const elementPrefix = "div.room-header-";
	if ($(`${elementPrefix}${roomNameCss} > .content > .users`).is(':visible') === false) {
		$(`${elementPrefix}${roomNameCss} > .header`).click();
	}
	$(`${elementPrefix}${roomNameCss}`)[0].scrollIntoView();
}
...
function autoJoiningHandler() {
	const AUTOJOIN_TIMEOUT_MS = 5000
	...
	clearTimeout(autoJoinTimer)
	autoJoinTimer = setTimeout(joinRooms, AUTOJOIN_TIMEOUT_MS)
}
